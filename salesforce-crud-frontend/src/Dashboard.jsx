...
export default function Dashboard() {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `HTTP ${response.status}: ${
              errorText || "Unable to fetch records"
            }`
          );
        }
  ...
  const handleNextPage = () => {
    if (!loading && hasMore) {
      setOffset(
        (previousOffset) => previousOffset + PAGE_SIZE
      );
    }
  };
  ...
  const handleSubmitForm = async (e) => {
    ...
    const url = isEditing
      ? `${SALESFORCE_API_URL}/${selectedObject}/${editingRecord.Id}`
      : `${SALESFORCE_API_URL}/${selectedObject}`;
  ...
  const handleDelete = async (recordId) => {
    ...
    console.log("URL:", `${SALESFORCE_API_URL}/${selectedObject}/${recordId}`);
    ...
    try {
      const response = await fetch(
        `${SALESFORCE_API_URL}/${selectedObject}/${recordId}`,
        {
          method: "DELETE",
          headers: {
            Accept: "application/json",
